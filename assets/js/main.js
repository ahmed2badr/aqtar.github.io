	...
	$.Angora_Theme = {
		...
		navigation:function() {
			...
			$('a[href^=\\#]').on("click", function(e) {
				if ($(this).attr('href')!=='#' && !$(e.target).parent().parent().is('.navbar-nav') && !$(this).attr('data-toggle')) {
					$(document).scrollTo($(this).attr('href'), Angora_Options.scrollSpeed, {offset:{top:-85, left:0}});
					e.preventDefault();
				}
			});
		...
		portfolio:function() {
			...
			$('a[href^="#view-"]').on("click", function() {
				...
				if ($item.length>0) {
					$(document).scrollTo('#portfolio', Angora_Options.scrollSpeed, {offset:{top:-85, left:0}, onAfter:function() {
						$item.trigger('click');
					}});
				}
			});
		...
		contact:function() {
			...
			$button.on("click", function() {
				...
				if ($message.val().length<1) 	{fieldNotice($message);}
...
function shareTo(network, title, image, url) {
	return $.Angora_Theme.share(network, title, image, url);
}
